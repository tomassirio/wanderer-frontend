...
help:
	...
	@echo "  make run-web-dev     - Run web app with dev environment"
	@echo "  make run-web-prod    - Run web app with prod environment"
...
# Run on Android emulator with production environment
run-android-prod:
	@echo "🤖 Running on Android emulator (production)..."
	@if [ ! -f .env.prod ]; then \
		echo "❌ Error: .env.prod file not found!"; \
		echo "Please create .env.prod from .env.template"; \
		exit 1; \
	fi
	@source .env.prod && \
	DEVICE_ID=$${ANDROID_DEVICE_ID:-emulator-5554} && \
...
# Run web application with production environment
run-web-prod:
